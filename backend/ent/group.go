...
type Group struct {
	...
	// 是否启用破限(shield)请求变换
	ShieldEnabled bool `json:"shield_enabled,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldShieldEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field shield_enabled", values[i])
			} else if value.Valid {
				_m.ShieldEnabled = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("shield_enabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.ShieldEnabled))
	builder.WriteString(", ")
